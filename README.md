...
## 🧪 Advanced Experimental Validations (New in V3.1)
...
* **Result:** As seen in the generated graph, the fluid-based retrospective torque achieves statistical correlations that strictly violate the linear Classical Bound (Markovian/Local), smoothly reproducing the quantum prediction via purely deterministic fluid mechanics.
...
* **Emitter Modulation:** The DWE photon emitter alternates between standard Gaussian distributions and asymmetric Airy packet distributions.
...
### 3. Run Experiment 1: Bell Test Violation

```bash
cargo run --release --bin exp1_bell

```

*Outputs: Simulates vortex fission and the non-Markovian retrospective torque, generating `result_bell_test.csv` with the correlation data across standard CHSH angles.*

### 4. Run Experiment 2: Airy Packet Dispersion

```bash
cargo run --release --bin exp2_airy

```

*Outputs: Evaluates the Quantum Magnus Effect and dispersion cancellation, generating `result_airy_vs_gauss.csv` mapping the spatial spread of Gaussian vs. Airy wave packets over time.*

### 5. Generate High-Resolution Analytics Plots (Python)

Once the CSV datasets are generated by the Rust engine, use the provided Python scripts in the `analytics/` folder to render publication-ready scientific graphs. Ensure you have the `pandas` and `matplotlib` libraries installed.
...
python plot_colapso.py

```

*Outputs: Reads the local `.csv` files and saves high-resolution `.png` images directly to the directory (e.g., `bell_test_violation.png`, `airy_packet_dispersion.png`, `hqpu_leitura_qnd.png`).*
